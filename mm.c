...
void mm_processor(double *a_in, double *b_in, double *a_out, double *b_out, int i, int j, double *element){
...
void mm_core(int n, double A[n], double B[n], double result[n][n]){
	int i = 0, j = 0, level = 0;
	...
	while(level <  total_level ){ //(n - 1) + (n - 1) + 1 
		...
		while(inc <= limit){
			if(inc == level - clevel && inc == clevel ){
				mm_processor(&a[level - inc][inc] ,&b[level - inc][inc] , 
				NULL, NULL,
...
/*
	for(int i = 0 ; i < (N<<1) - 1 ; i ++){
		mm_core(N, A[ i ], B[i], result ) ;
	}
*/
	for(int i = 0; i < 3; i ++){
		mm_core(N, AA[i] , BB[i] , result ) ;
	}
	for(int i = 0; i < 3 ; i ++){
		for(int j = 0; j < 3; j ++){
			printf("%f\t", result[i][j] ) ;
